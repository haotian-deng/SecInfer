...
int64_t* ReLU(int64_t* input, mo::PartyPointer& party, HE<MultiIOBase>* he, uint32_t num, int l, int module_p,
  ...
  for (std::size_t i = 0; i < val[0].size(); ++i) {
    ...
    // 3. Convert to its true value
    auto tmp = comparison_result * module_p;  // Boolean sharing DOT a const value
    sum = comparison_result.Mux((tmp - sum).Get(), sum.Get());

    // 4. Truncate and map to Field
    sum = sum.RightShiftWithFill(5, comparison_result, p);
...
int64_t* ForwardAlexNet(mo::PartyPointer& party, int64_t* input, uint32_t num, int l, int module_p, ThreadPool* pool,
                        HE<MultiIOBase>* he, MPIOChannel<MultiIOBase>* io, int height, int width) {
  // AlexNet 的各层结构：
  ...
  // auto pool1 = conv1;
  auto start = clock_start();
  auto pool1 = PoolLayer(height, width, conv1, module_p, party, he, num, l, pool, io, a2b_converter, b2a_converter);
  auto timeused = time_from(start);
  total_time = timeused;
...
int main(int ac, char* av[]) {
  ...
  uint32_t height = user_options["height"].as<std::uint32_t>();
  uint32_t width = user_options["width"].as<std::uint32_t>();
  ...
  // 前向传播
  int64_t* final_output = ForwardAlexNet(party, inputs, num, l, module_p, &pool, he, io, height, width);
...
std::pair<program_options::variables_map, bool> ParseProgramOptions(int ac, char* av[]) {
  ...
  description.add_options()
      ...
      ("height,i", program_options::value<uint32_t>(), "This party's input")
      ("width,i", program_options::value<uint32_t>(), "This party's input")
